        ...
        <>
            ...
            <div style={mobile_menu_css}>
                <div style="display: flex; flex-direction: column; align-items: center; gap: 12px; margin-bottom: 10px;">
                    ...
                    <div style="font-family: 'JetBrains Mono'; font-weight: 900; color: var(--primary); font-size: 0.8rem; letter-spacing: 2px; text-align: center;">{ "NVIDIA x NCHU x MSI" }</div>
                </div>
                ...
                <button onclick={props.on_toggle.clone()} style="margin-top: 10px; background: transparent; border: 2px solid var(--primary); color: var(--primary); padding: 10px 40px; border-radius: 30px; font-weight: 900; cursor: pointer;">{ if is_en { "SWITCH TO 中文" } else { "切換至 ENGLISH" } }</button>
            </div>
        </>
    }
}

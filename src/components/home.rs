...
pub fn home(props: &PageProps) -> Html {
    ...
    html! {
        <div style="min-height: 100vh; display: flex; flex-direction: column; justify-content: center; align-items: center; text-align: center; padding: 120px 20px 40px;">
            <h1 class={*animate_class} style="font-size: clamp(2.8rem, 10vw, 6.5rem); font-weight: 900; letter-spacing: -2px; background: linear-gradient(to bottom, #fff, #666); -webkit-background-clip: text; -webkit-text-fill-color: transparent; line-height: 1.1; margin-bottom: 15px;">
                { "LIN SHU-JEN" }
            </h1>
            ...
            <p style="color: #888; max-width: 650px; line-height: 1.8; font-size: 1.05rem;">
                { if is_en { "Specializing in Digital Twins & Embodied AI. Collaborative partner with MSI Innovation Center. Powered by Rust & Wasm." } 
                  else { "專注於數位孿生與具身智能。微星科技創新前瞻中心合作夥伴。採用 Rust 與 WebAssembly 技術。" } }
            </p>
        </div>
    }
}

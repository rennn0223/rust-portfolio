        ...
        <div class="container" style="padding: 100px 24px;">
            ...
            <div style="display: grid; grid-template-columns: repeat(auto-fit, minmax(320px, 1fr)); gap: 30px; align-items: start;">
                ...
                <div class={card_class} style={card_style}>
                    ...
                    if *exp_1 {
                        <div style="margin-top: 10px; padding-top: 15px; border-top: 1px solid rgba(255,255,255,0.1); color: #ccc; font-size: 0.9rem; line-height: 1.7;">
                            { if is_en { 
                                "Lead Architect for the Campus Inspection System. Developed a real-time Digital Twin environment for autonomous monitoring using Omniverse. Collaborated with MSI to deploy on DGX Spark, ensuring sub-millisecond synchronization. Personally recognized by CEO Jensen Huang." 
                            } else { 
                                "擔任校園巡檢系統首席架構師。使用 Omniverse 開發用於自主監控的即時數位孿生環境。與微星團隊合作部署於 DGX Spark，確保亞毫秒級的虛實同步，專案榮獲執行長黃仁勳親筆肯定。" 
                            } }
                        </div>
                    }
                ...
                <div class={card_class} style={card_style}>
                    ...
                    if *exp_2 {
                        <div style="margin-top: 10px; padding-top: 15px; border-top: 1px solid rgba(255,255,255,0.1); color: #ccc; font-size: 0.9rem; line-height: 1.7;">
                            { if is_en { 
                                "Technical exchange focusing on ROS2 control and Embodied Intelligence integration after GTC 2026." 
                            } else { 
                                "GTC 2026 展後進行技術交流，專注於 ROS2 控制與具身智能整合。" 
                            } }
                        </div>
                    }
                ...
                <div class={card_class} style={card_style}>
                    ...
                    if *exp_3 {
                        <div style="margin-top: 10px; padding-top: 15px; border-top: 1px solid rgba(255,255,255,0.1); color: #ccc; font-size: 0.9rem; line-height: 1.7;">
                            { if is_en { 
                                "Architected industrial-grade Digital Twin pipelines and managed MGX server hardware upgrades." 
                            } else { 
                                "建構工業級數位孿生流水線，並負責 MGX 伺服器硬體升級。" 
                            } }
                        </div>
                    }
